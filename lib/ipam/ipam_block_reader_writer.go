...
//zk modify findUnclaimedBlock logic
func (rw blockReaderWriter) findUnclaimedBlock(ctx context.Context, host string, version ipVersion, pool cnet.IPNet, config IPAMConfig) (*cnet.IPNet, error) {
	...
	log.Debugf("Getting block: %s", pool.String())
	key := model.BlockKey{CIDR: *pool}
	_, err := rw.client.Get(ctx, key, "")
	if err != nil {
		if _, ok := err.(cerrors.ErrorResourceDoesNotExist); ok {
			log.Infof("Found free block: %+v", *pool)
			return pool, nil
		}
	...
	return nil, BlocksExist("This Block already exist,try others")
}
