...
//zk determinePools() add hostname field
func (c ipamClient) determinePools(requestedPools []net.IPNet, version ipVersion, host string) (map[string][]net.IPNet, error) {
	enabledPools, err := c.pools.GetEnabledPools(version.Number, host)
	if err != nil {
...
//zk  Match the IP pool by adding IP fields  add hostname in determinePools(requestedPools, version)
func (c ipamClient) autoAssign(ctx context.Context, num int, handleID *string, attrs map[string]string, requestedPools []net.IPNet, version ipVersion, host string) ([]net.IP, error) {
	...
	pools, err := c.determinePools(requestedPools, version, host)
	if err != nil {
		return nil, err
	}
	...
	//kun modify getAffineBlocks
	//affBlocks, err := c.blockReaderWriter.getAffineBlocks(ctx, host, version, pools)
	for poolname, poolvalue := range pools {
		affBlocks, err := c.blockReaderWriter.getAffineBlocks(ctx, poolname, version, poolvalue)
		if err != nil {
			return nil, err
		}
	}
	...
	for len(ips) < num {
		...
		for i := 0; i < ipamEtcdRetries; i++ {
			...
			aff, err := c.client.Get(ctx, model.BlockAffinityKey{Host: poolname, CIDR: cidr}, "")
			if err != nil {
	...
	if config.AutoAllocateBlocks == true {
		...
		for rem > 0 && retries > 0 {
			...
			//zk
			for poolname, poolvalue := range pools {
				subnet, err := c.blockReaderWriter.findUnclaimedBlock(ctx, host, version, pool, *config)
				if err != nil {
					if _, ok := err.(BlocksExist); ok {
						// No free blocks.  Break.
						logCtx.Info("This Block already exist,try others")
						continue
					}
				...
				for i := 0; i < ipamEtcdRetries; i++ {
					...
					pa, err := c.blockReaderWriter.getPendingAffinity(ctx, poolname, *subnet)
					if err != nil {
						if _, ok := err.(cerrors.ErrorResourceUpdateConflict); ok {
					...
					newIPs, err := c.assignFromExistingBlock(ctx, b, rem, handleID, attrs, poolname, config.StrictAffinity)
					if err != nil {
						if _, ok := err.(cerrors.ErrorResourceUpdateConflict); ok {
	...
	if config.StrictAffinity != true && rem != 0 {
		...
		for poolname, poolvalue := range pools {
			...
			for rem > 0 {
				...
				for i := 0; i < ipamEtcdRetries; i++ {
					b, err := c.client.Get(ctx, model.BlockKey{CIDR: *poolvalue}, "")
					if err != nil {
					...
					logCtx.Infof("Attempting to assign IPs from non-affine block %s", poolvalue.String())
					newIPs, err := c.assignFromExistingBlock(ctx, b, rem, handleID, attrs, poolname, false)
					if err != nil {
						if _, ok := err.(cerrors.ErrorResourceUpdateConflict); ok {
					...
					logCtx.Infof("Successfully assigned IPs from non-affine block %s", poolvalue.String())
					ips = append(ips, newIPs...)
					rem = num - len(ips)
					break
				}
