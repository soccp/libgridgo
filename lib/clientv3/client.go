...
//zk modify GetEnablePools add host field and return pool map with poolname
func (p poolAccessor) GetEnabledPools(ipVersion int, host string) (map[string][]net.IPNet, error) {
	pools, err := p.client.IPPools().List(context.Background(), options.ListOptions{})
	if err != nil {
		return nil, err
	}
	log.Debugf("Got list of all IPPools: %v", pools)
	enabled := map[string][]net.IPNet{}
	for _, pool := range pools.Items {
		if pool.Spec.Disabled {
			continue
		} else if _, cidr, err := net.ParseCIDR(pool.Spec.CIDR); err == nil && cidr.Version() == ipVersion {
			if matchPool(host, pool.Spec.(metav1.ObjectMeta).Name) {
				log.Debugf("Adding pool (%s) to the enabled IPPool list", cidr.String())
				//enabled = append(enabled, *cidr)
				enabled[pool.Spec.(metav1.ObjectMeta).Name] = *cidr
			} else {
				log.Debugf("Ignoring IPPool: %s. does not match host: %s", pool.Spec.CIDR, host)
			}
		} else if err != nil {
